import { Injectable, signal, Signal } from '@angular/core';

export type Lang = 'fr' | 'en';

const LANG_KEY = 'grandtour.lang.v1';

const FR: Record<string, string> = {
  // Topbar / nav
  'header.subtitle': 'DRAGON BALL SUPER CARD GAME MASTERS EU Circuit',
  'header.subtitleShort': 'DBSCG EU Circuit',
  'nav.map': 'Carte',
  'nav.calendar': 'Calendrier',
  'nav.season': 'Ma saison',
  'nav.collection': 'Collections',
  'nav.budget': 'Budget',
  'nav.dashboard': 'Dashboard',
  'nav.admin': 'Admin',
  'nav.login': 'Connexion',
  'nav.register': 'Inscription',
  'nav.logout': 'Déconnexion',
  'nav.greeting': 'Bonjour',
  'nav.profileTooltip': 'Voir mon profil',

  // Admin events
  'admin.title': 'Administration des events',
  'admin.addButton': '+ Nouvel event',
  'admin.editButton': 'Modifier',
  'admin.deleteButton': 'Supprimer',
  'admin.deleteConfirm': 'Supprimer cet event ? Cette action est irréversible.',
  'admin.empty': 'Aucun event pour l\'instant.',
  'admin.form.createTitle': 'Nouvel event',
  'admin.form.editTitle': 'Modifier l\'event',
  'admin.form.name': 'Nom',
  'admin.form.type': 'Type',
  'admin.form.typeRegional': 'Regional',
  'admin.form.typeFinals': 'Finals',
  'admin.form.date': 'Date',
  'admin.form.city': 'Ville',
  'admin.form.country': 'Pays',
  'admin.form.venue': 'Lieu',
  'admin.form.lat': 'Latitude',
  'admin.form.lng': 'Longitude',
  'admin.form.registerLink': 'Lien d\'inscription',
  'admin.form.registerLinkOptional': 'optionnel',
  'admin.form.cancel': 'Annuler',
  'admin.form.submitCreate': 'Créer',
  'admin.form.submitUpdate': 'Enregistrer',
  'admin.form.busy': 'Enregistrement…',
  'admin.errors.fieldRequired': 'Ce champ est requis.',
  'admin.errors.generic': 'Une erreur est survenue, réessaie.',

  // Login page
  'login.title': 'Connexion',
  'login.subtitle': 'Reprends ton Grand Tour.',
  'login.email': 'Email',
  'login.password': 'Mot de passe',
  'login.submit': 'Se connecter',
  'login.busy': 'Connexion…',
  'login.switch': 'Pas encore inscrit ?',
  'login.switchLink': 'Créer un compte',
  'login.forgotLink': 'Mot de passe oublié ?',
  'login.errors.emailRequired': 'Email requis.',
  'login.errors.emailInvalid': 'Email invalide.',
  'login.errors.passwordRequired': 'Mot de passe requis.',
  'login.errors.invalid': 'Email ou mot de passe incorrect.',
  'login.errors.emailNotConfirmed': 'Email non confirmé.',
  'login.errors.resendLink': 'Renvoyer le lien de confirmation',
  'login.errors.resendDone': 'Lien renvoyé !',

  // Register page
  'register.title': 'Inscription',
  'register.subtitle': 'Rejoins le Grand Tour.',
  'register.username': 'Username',
  'register.email': 'Email',
  'register.password': 'Mot de passe',
  'register.passwordConfirm': 'Confirmer le mot de passe',
  'register.bandai': 'Bandai TCG+',
  'register.bandaiOptional': 'Facultatif',
  'register.bandaiHint': 'Ton identifiant numérique sur l\'app Bandai TCG+.',
  'register.submit': 'Créer mon compte',
  'register.busy': 'Création…',
  'register.switch': 'Déjà inscrit ?',
  'register.switchLink': 'Se connecter',
  'register.errors.usernameRequired': 'Username requis.',
  'register.errors.usernameLength': 'Le username doit faire entre 3 et 20 caractères.',
  'register.errors.usernamePattern': 'Caractères autorisés : lettres, chiffres, _ et -.',
  'register.errors.usernameTaken': 'Ce username est déjà pris.',
  'register.errors.emailRequired': 'Email requis.',
  'register.errors.emailInvalid': 'Email invalide.',
  'register.errors.emailTaken': 'Cet email est déjà utilisé.',
  'register.errors.passwordRequired': 'Mot de passe requis.',
  'register.errors.passwordMinLength': 'Mot de passe d\'au moins 8 caractères.',
  'register.errors.confirmRequired': 'Confirmation requise.',
  'register.errors.passwordsMismatch': 'Les mots de passe ne correspondent pas.',
  'register.errors.bandaiInvalid': 'L\'ID Bandai TCG+ doit contenir entre 8 et 12 chiffres.',
  'register.errors.invalidFields': 'Champs invalides.',

  // Register — verify pending screen
  'register.verifyPending.title': 'Vérifie ton email',
  'register.verifyPending.body': 'Un lien de confirmation a été envoyé à {email}. Clique dessus pour activer ton compte, puis reviens te connecter.',
  'register.verifyPending.checkSpam': 'Pense à vérifier tes spams si tu ne le vois pas.',
  'register.verifyPending.resend': 'Renvoyer le lien',
  'register.verifyPending.resending': 'Envoi…',
  'register.verifyPending.resendDone': 'Lien renvoyé !',
  'register.verifyPending.backToLogin': 'Aller à la connexion',

  // Forgot password page
  'forgot.title': 'Mot de passe oublié',
  'forgot.subtitle': 'Indique ton email, on t\'envoie un lien pour définir un nouveau mot de passe.',
  'forgot.email': 'Email',
  'forgot.submit': 'Envoyer le lien',
  'forgot.busy': 'Envoi…',
  'forgot.backToLogin': '← Retour à la connexion',
  'forgot.errors.emailRequired': 'Email requis.',
  'forgot.errors.emailInvalid': 'Email invalide.',
  'forgot.linkSent.title': 'C\'est parti',
  'forgot.linkSent.body': 'Si un compte existe avec cet email, tu vas recevoir un lien dans quelques instants.',

  // Reset password page (post-link)
  'reset.title': 'Nouveau mot de passe',
  'reset.body': 'Définis un nouveau mot de passe pour ton compte.',
  'reset.newPassword': 'Nouveau mot de passe',
  'reset.confirmPassword': 'Confirmer',
  'reset.submit': 'Enregistrer',
  'reset.busy': 'Enregistrement…',
  'reset.errors.required': 'Mot de passe requis.',
  'reset.errors.minLength': '8 caractères minimum.',
  'reset.errors.passwordsMismatch': 'Les mots de passe ne correspondent pas.',
  'reset.errors.invalidToken': 'Le lien a expiré ou est invalide. Demande un nouveau lien.',
  'reset.errors.invalidFields': 'Champs invalides.',
  'reset.success.title': 'Mot de passe mis à jour',
  'reset.success.body': 'Redirection en cours…',

  // Verified screen (post email confirm)
  'verified.loading': 'Confirmation en cours…',
  'verified.title': 'Compte vérifié !',
  'verified.titleWithName': 'Bienvenue {username} !',
  'verified.body': 'Ton compte est actif. Tu peux te connecter.',
  'verified.cta': 'Aller à la connexion',
  'verified.error.title': 'Lien invalide',
  'verified.error.body': 'Ce lien de confirmation est invalide ou a expiré.',
  'verified.error.cta': 'Retour à la connexion',

  // Signup CTA banner (public pages)
  'signupCta.body': 'Tu joues le circuit ? Crée ton compte pour suivre tes events, résultats et dépenses.',
  'signupCta.cta': 'Créer mon compte',

  // Map view
  'map.legendTitle': 'Grand Tour',
  'map.eventsCount': '{count} events sur la saison',
  'map.legendRegional': 'Regional',
  'map.legendFinals': 'Finals',
  'map.legendRegistered': 'Inscrit',

  // Event card
  'event.date': 'Date',
  'event.city': 'Ville',
  'event.venue': 'Lieu',
  'event.register': '+ Je participe',
  'event.registered': '✓ J\'y participe',
  'event.officialLink': 'S\'inscrire sur le site officiel ↗',
  'event.close': 'Fermer',

  // Calendar page
  'calendar.title': 'Calendrier',
  'calendar.subtitle': 'Tous les events de la saison.',
  'calendar.count': '{count} event sur la sélection',
  'calendar.countPlural': '{count} events sur la sélection',
  'calendar.empty': 'Aucun event ne correspond à tes filtres.',
  'calendar.filter.type.label': 'Type',
  'calendar.filter.type.all': 'Tous',
  'calendar.filter.country.label': 'Pays',
  'calendar.filter.country.all': 'Tous les pays',
  'calendar.filter.period.label': 'Période',
  'calendar.filter.period.all': 'Tous',
  'calendar.filter.period.upcoming': 'À venir',
  'calendar.filter.period.past': 'Passés',
  'calendar.filter.reset': 'Réinitialiser',
  'calendar.search.label': 'Recherche',
  'calendar.search.placeholder': 'Nom d\'event ou ville…',
  'calendar.search.clear': 'Effacer',

  // My Season page
  'season.title': 'Ma saison',
  'season.subtitle': 'Les events que tu as marqués comme "je participe".',
  'season.upcoming.title': 'À venir',
  'season.past.title': 'Passés',
  'season.empty.upcoming': 'Aucun event à venir. Marque "Je participe" sur la carte ou le calendrier.',
  'season.empty.past': 'Aucun event passé pour l\'instant.',

  // Event detail page (Phase 3)
  'detail.back': '← Retour',
  'detail.detailButton': 'Mon bilan',
  'detail.section.summary': 'Bilan',
  'detail.section.result': 'Résultat',
  'detail.section.matches': 'Matchs',
  'detail.section.expenses': 'Dépenses',
  'detail.notFound': 'Event introuvable.',
  'detail.summary.spent': 'Dépensé',
  'detail.summary.prizes': 'Gains',
  'detail.summary.net': 'Bilan net',
  'detail.summary.noResult': 'Aucun résultat enregistré pour cet event.',
  'detail.result.placement': 'Placement',
  'detail.result.totalPlayers': 'Joueurs au total',
  'detail.result.deckName': 'Nom du deck',
  'detail.result.leaderPlayed': 'Leader joué',
  'leaderPicker.searchPlaceholder': 'Rechercher un leader…',
  'leaderPicker.change': 'Changer',
  'leaderPicker.viewNormalFace': 'Face normale',
  'leaderPicker.viewAwakenedFace': 'Face éveillée',
  'detail.result.prizes': 'Gains (€)',
  'detail.result.notes': 'Notes',
  'detail.result.save': 'Enregistrer',
  'detail.result.saving': 'Enregistrement…',
  'detail.result.saved': 'Résultat enregistré.',
  'detail.result.delete': 'Effacer le résultat',
  'detail.result.deleteConfirm': 'Effacer le résultat (et tous les matchs) ?',
  'detail.result.errors.required': 'Champs obligatoires manquants.',
  'detail.result.errors.deckLeaderConflict': 'Ce nom de deck est déjà utilisé avec le leader {leader} — choisissez un autre nom, ou sélectionnez ce leader.',
  'detail.matches.empty': 'Aucun match enregistré pour l\'instant.',
  'detail.matches.add': 'Ajouter un match',
  'detail.matches.round': 'Round',
  'detail.matches.opponentLeader': 'Leader adverse',
  'detail.matches.opponentName': 'Pseudo adverse',
  'detail.matches.result': 'Résultat',
  'detail.matches.result.win': 'Victoire',
  'detail.matches.result.loss': 'Défaite',
  'detail.matches.result.draw': 'Égalité',
  'detail.matches.result.bye': 'Bye',
  'detail.matches.record': '{w}V – {l}D – {d}N – {b}B',
  'detail.matches.delete': 'Supprimer',
  'detail.matches.errors.required': 'Leader et résultat requis.',
  'detail.matches.cta': 'Ajouter',
  'detail.matches.update': 'Enregistrer',
  'detail.matches.cancelEdit': 'Annuler',
  'detail.expenses.empty': 'Aucune dépense.',
  'detail.expenses.add': 'Ajouter une dépense',
  'detail.expenses.category': 'Catégorie',
  'detail.expenses.amount': 'Montant (€)',
  'detail.expenses.notes': 'Note',
  'detail.expenses.total': 'Total dépensé',
  'detail.expenses.delete': 'Supprimer',
  'detail.expenses.errors.required': 'Catégorie et montant requis.',
  'detail.expenses.cta': 'Ajouter',
  'detail.expenses.cat.Transport': 'Transport',
  'detail.expenses.cat.Hotel': 'Hôtel',
  'detail.expenses.cat.Entry': 'Inscription',
  'detail.expenses.cat.Food': 'Nourriture',
  'detail.expenses.cat.Other': 'Autre',

  // Collections
  'collection.list.title': 'Mes collections',
  'collection.list.add': 'Nouvelle collection',
  'collection.list.empty': 'Aucune collection pour l\'instant.',
  'collection.list.cards': '{count} cartes',
  'collection.editor.back': '← Retour',
  'collection.editor.namePlaceholder': 'Nom de la collection',
  'collection.editor.myCollection': 'Ma collection',
  'collection.editor.loading': 'Chargement…',
  'collection.editor.summary': '{count} cartes · {total} €',
  'collection.editor.empty': 'Aucune carte ajoutée.',
  'collection.editor.save': 'Enregistrer la collection',
  'collection.detail.loading': 'Chargement…',
  'collection.detail.notFound': 'Collection introuvable.',
  'collection.detail.edit': 'Modifier',
  'collection.detail.delete': 'Supprimer',
  'collection.detail.deleteConfirm': 'Supprimer cette collection ? Cette action est irréversible.',
  'collection.detail.summary': '{count} cartes · {total} €',
  'collection.detail.empty': 'Cette collection ne contient aucune carte.',
  'collection.errors.nameRequired': 'Le nom de la collection est requis.',
  'collection.errors.saveFailed': 'Une erreur est survenue, réessaie.',
  'cardGrid.searchPlaceholder': 'Rechercher une carte…',
  'cardGrid.allTypes': 'Tous les types',
  'cardGrid.allColors': 'Toutes les couleurs',
  'cardGrid.allSeries': 'Toutes les séries',
  'cardGrid.allRarities': 'Toutes les raretés',
  'cardGrid.empty': 'Aucune carte trouvée.',
  'cardGrid.prev': 'Précédent',
  'cardGrid.next': 'Suivant',
  'cardDetail.quantity': 'Quantité',
  'cardDetail.price': 'Prix unitaire (€)',
  'cardDetail.add': 'Ajouter',
  'cardDetail.remove': 'Retirer',
  'cardDetail.language': 'Langue',
  'cardDetail.languageUnset': 'Non précisée',
  'cardDetail.languageFr': 'Français',
  'cardDetail.languageEn': 'Anglais',
  'cardDetail.viewNormalFace': 'Face normale',
  'cardDetail.viewAwakenedFace': 'Face éveillée',

  // Profile page
  'profile.title': 'Mon profil',
  'profile.subtitle': 'Tes informations Grand Tour.',
  'profile.section.account': 'Compte',
  'profile.section.meta': 'Informations',
  'profile.section.danger': 'Zone sensible',
  'profile.username': 'Username',
  'profile.email': 'Email',
  'profile.bandai': 'Bandai TCG+',
  'profile.bandaiOptional': 'Facultatif',
  'profile.createdAt': 'Membre depuis',
  'profile.changePassword': 'Changer mon mot de passe',
  'profile.save': 'Enregistrer',
  'profile.busy': 'Enregistrement…',
  'profile.successUpdated': 'Profil mis à jour.',
  'profile.delete': 'Supprimer mon compte',
  'profile.deleteConfirm': 'Supprimer définitivement ton compte ? Cette action est irréversible.',
  'profile.logout': 'Se déconnecter',
  'profile.errors.usernameRequired': 'Username requis.',
  'profile.errors.usernameLength': 'Le username doit faire entre 3 et 20 caractères.',
  'profile.errors.usernamePattern': 'Caractères autorisés : lettres, chiffres, _ et -.',
  'profile.errors.usernameTaken': 'Ce username est déjà pris.',
  'profile.errors.emailRequired': 'Email requis.',
  'profile.errors.emailInvalid': 'Email invalide.',
  'profile.errors.emailTaken': 'Cet email est déjà utilisé.',
  'profile.errors.bandaiInvalid': 'L\'ID Bandai TCG+ doit contenir entre 8 et 12 chiffres.',
  'profile.errors.invalidFields': 'Champs invalides.',

  // Budget page (Phase 4)
  'budget.title': 'Budget',
  'budget.subtitle': 'Bilan agrégé de tes dépenses et gains sur la saison.',
  'budget.year': 'Année',
  'budget.totals.spent': 'Dépensé',
  'budget.totals.prizes': 'Gains',
  'budget.totals.net': 'Net',
  'budget.byCategory.title': 'Par catégorie',
  'budget.byCategory.empty': 'Aucune dépense pour cette année.',
  'budget.byEvent.title': 'Par événement',
  'budget.byEvent.empty': 'Aucun événement budgétisé pour cette année.',
  'budget.byEvent.col.event': 'Event',
  'budget.byEvent.col.date': 'Date',
  'budget.byEvent.col.spent': 'Dépensé',
  'budget.byEvent.col.prizes': 'Gains',
  'budget.byEvent.col.net': 'Net',

  // Dashboard page (Phase 4)
  'dashboard.title': 'Dashboard',
  'dashboard.subtitle': 'Ta saison en un coup d\'œil.',
  'dashboard.year': 'Année',
  'dashboard.empty': 'Aucune donnée pour cette année. Ajoute un résultat sur la fiche d\'un event.',
  'dashboard.wr.title': 'Win rate global',
  'dashboard.wr.played': '{count} matchs joués',
  'dashboard.wr.record': '{w}V – {l}D – {d}N – {b}B',
  'dashboard.best.title': 'Meilleur classement',
  'dashboard.best.empty': 'Aucun résultat enregistré pour l\'instant.',
  'dashboard.best.outOf': 'sur {total}',
  'dashboard.cities.title': 'Villes visitées',
  'dashboard.cities.count': '{cities} villes · {countries} pays',
  'dashboard.cities.empty': 'Aucun event joué cette année.',
  'dashboard.matchup.title': 'Matchup breakdown',
  'dashboard.matchup.deckFilter': 'Mon leader',
  'dashboard.matchup.all': 'Tous',
  'dashboard.matchup.empty': 'Aucun match pour ce leader.',
  'dashboard.matchup.col.opp': 'Adversaire',
  'dashboard.matchup.col.n': 'N',
  'dashboard.matchup.col.wr': 'WR',
  'dashboard.deck.title': 'Win rate par deck',
  'dashboard.deck.empty': 'Aucun deck joué cette année.',
  'dashboard.deck.col.deck': 'Deck',
  'dashboard.deck.col.leader': 'Leader',
  'dashboard.deck.col.n': 'N',
  'dashboard.deck.col.wr': 'WR',
  'dashboard.money.title': 'Bilan financier',
  'dashboard.money.seeDetail': 'Voir le détail →',
  'dashboard.next.title': 'Prochains events',
  'dashboard.next.empty': 'Aucun event à venir.',
};

const EN: Record<string, string> = {
  // Topbar / nav
  'header.subtitle': 'DRAGON BALL SUPER CARD GAME MASTERS EU Circuit',
  'header.subtitleShort': 'DBSCG EU Circuit',
  'nav.map': 'Map',
  'nav.calendar': 'Calendar',
  'nav.season': 'My season',
  'nav.collection': 'Collections',
  'nav.budget': 'Budget',
  'nav.dashboard': 'Dashboard',
  'nav.admin': 'Admin',
  'nav.login': 'Sign in',
  'nav.register': 'Sign up',
  'nav.logout': 'Sign out',
  'nav.greeting': 'Hi',
  'nav.profileTooltip': 'View my profile',

  // Admin events
  'admin.title': 'Manage events',
  'admin.addButton': '+ New event',
  'admin.editButton': 'Edit',
  'admin.deleteButton': 'Delete',
  'admin.deleteConfirm': 'Delete this event? This cannot be undone.',
  'admin.empty': 'No events yet.',
  'admin.form.createTitle': 'New event',
  'admin.form.editTitle': 'Edit event',
  'admin.form.name': 'Name',
  'admin.form.type': 'Type',
  'admin.form.typeRegional': 'Regional',
  'admin.form.typeFinals': 'Finals',
  'admin.form.date': 'Date',
  'admin.form.city': 'City',
  'admin.form.country': 'Country',
  'admin.form.venue': 'Venue',
  'admin.form.lat': 'Latitude',
  'admin.form.lng': 'Longitude',
  'admin.form.registerLink': 'Registration link',
  'admin.form.registerLinkOptional': 'optional',
  'admin.form.cancel': 'Cancel',
  'admin.form.submitCreate': 'Create',
  'admin.form.submitUpdate': 'Save',
  'admin.form.busy': 'Saving…',
  'admin.errors.fieldRequired': 'This field is required.',
  'admin.errors.generic': 'Something went wrong, try again.',

  // Login page
  'login.title': 'Sign in',
  'login.subtitle': 'Continue your Grand Tour.',
  'login.email': 'Email',
  'login.password': 'Password',
  'login.submit': 'Sign in',
  'login.busy': 'Signing in…',
  'login.switch': 'No account yet?',
  'login.switchLink': 'Create one',
  'login.forgotLink': 'Forgot password?',
  'login.errors.emailRequired': 'Email required.',
  'login.errors.emailInvalid': 'Invalid email.',
  'login.errors.passwordRequired': 'Password required.',
  'login.errors.invalid': 'Invalid email or password.',
  'login.errors.emailNotConfirmed': 'Email not confirmed yet.',
  'login.errors.resendLink': 'Resend the confirmation link',
  'login.errors.resendDone': 'Link sent!',

  // Register page
  'register.title': 'Sign up',
  'register.subtitle': 'Join the Grand Tour.',
  'register.username': 'Username',
  'register.email': 'Email',
  'register.password': 'Password',
  'register.passwordConfirm': 'Confirm password',
  'register.bandai': 'Bandai TCG+',
  'register.bandaiOptional': 'Optional',
  'register.bandaiHint': 'Your numeric ID on the Bandai TCG+ app.',
  'register.submit': 'Create account',
  'register.busy': 'Creating…',
  'register.switch': 'Already registered?',
  'register.switchLink': 'Sign in',
  'register.errors.usernameRequired': 'Username required.',
  'register.errors.usernameLength': 'Username must be 3 to 20 characters.',
  'register.errors.usernamePattern': 'Allowed: letters, digits, _ and -.',
  'register.errors.usernameTaken': 'This username is already taken.',
  'register.errors.emailRequired': 'Email required.',
  'register.errors.emailInvalid': 'Invalid email.',
  'register.errors.emailTaken': 'This email is already used.',
  'register.errors.passwordRequired': 'Password required.',
  'register.errors.passwordMinLength': 'Password must be at least 8 characters.',
  'register.errors.confirmRequired': 'Confirmation required.',
  'register.errors.passwordsMismatch': 'Passwords do not match.',
  'register.errors.bandaiInvalid': 'Bandai TCG+ ID must be 8 to 12 digits.',
  'register.errors.invalidFields': 'Invalid fields.',

  // Register — verify pending screen
  'register.verifyPending.title': 'Verify your email',
  'register.verifyPending.body': 'A confirmation link has been sent to {email}. Click it to activate your account, then come back to sign in.',
  'register.verifyPending.checkSpam': 'Check your spam folder if you don\'t see it.',
  'register.verifyPending.resend': 'Resend link',
  'register.verifyPending.resending': 'Sending…',
  'register.verifyPending.resendDone': 'Link sent!',
  'register.verifyPending.backToLogin': 'Go to sign in',

  // Profile page
  'profile.title': 'My profile',
  'profile.subtitle': 'Your Grand Tour account.',
  'profile.section.account': 'Account',
  'profile.section.meta': 'Information',
  'profile.section.danger': 'Danger zone',
  'profile.username': 'Username',
  'profile.email': 'Email',
  'profile.bandai': 'Bandai TCG+',
  'profile.bandaiOptional': 'Optional',
  'profile.createdAt': 'Member since',
  'profile.changePassword': 'Change password',
  'profile.save': 'Save',
  'profile.busy': 'Saving…',
  'profile.successUpdated': 'Profile updated.',
  'profile.delete': 'Delete my account',
  'profile.deleteConfirm': 'Permanently delete your account? This cannot be undone.',
  'profile.logout': 'Sign out',
  'profile.errors.usernameRequired': 'Username required.',
  'profile.errors.usernameLength': 'Username must be 3 to 20 characters.',
  'profile.errors.usernamePattern': 'Allowed: letters, digits, _ and -.',
  'profile.errors.usernameTaken': 'This username is already taken.',
  'profile.errors.emailRequired': 'Email required.',
  'profile.errors.emailInvalid': 'Invalid email.',
  'profile.errors.emailTaken': 'This email is already used.',
  'profile.errors.bandaiInvalid': 'Bandai TCG+ ID must be 8 to 12 digits.',
  'profile.errors.invalidFields': 'Invalid fields.',

  // Forgot password page
  'forgot.title': 'Forgot password',
  'forgot.subtitle': 'Enter your email — we\'ll send you a link to set a new password.',
  'forgot.email': 'Email',
  'forgot.submit': 'Send link',
  'forgot.busy': 'Sending…',
  'forgot.backToLogin': '← Back to sign in',
  'forgot.errors.emailRequired': 'Email required.',
  'forgot.errors.emailInvalid': 'Invalid email.',
  'forgot.linkSent.title': 'Done',
  'forgot.linkSent.body': 'If an account exists with this email, you\'ll receive a link shortly.',

  // Reset password page (post-link)
  'reset.title': 'New password',
  'reset.body': 'Set a new password for your account.',
  'reset.newPassword': 'New password',
  'reset.confirmPassword': 'Confirm',
  'reset.submit': 'Save',
  'reset.busy': 'Saving…',
  'reset.errors.required': 'Password required.',
  'reset.errors.minLength': '8 characters minimum.',
  'reset.errors.passwordsMismatch': 'Passwords do not match.',
  'reset.errors.invalidToken': 'The link is expired or invalid. Request a new one.',
  'reset.errors.invalidFields': 'Invalid fields.',
  'reset.success.title': 'Password updated',
  'reset.success.body': 'Redirecting…',

  // Verified screen (post email confirm)
  'verified.loading': 'Confirming…',
  'verified.title': 'Account verified!',
  'verified.titleWithName': 'Welcome {username}!',
  'verified.body': 'Your account is active. You can sign in.',
  'verified.cta': 'Go to sign in',
  'verified.error.title': 'Invalid link',
  'verified.error.body': 'This confirmation link is invalid or has expired.',
  'verified.error.cta': 'Back to sign in',

  // Signup CTA banner (public pages)
  'signupCta.body': 'Playing the circuit? Create your account to track events, results and expenses.',
  'signupCta.cta': 'Create account',

  // Map view
  'map.legendTitle': 'Grand Tour',
  'map.eventsCount': '{count} events this season',
  'map.legendRegional': 'Regional',
  'map.legendFinals': 'Finals',
  'map.legendRegistered': 'Registered',

  // Event card
  'event.date': 'Date',
  'event.city': 'City',
  'event.venue': 'Venue',
  'event.register': '+ Register',
  'event.registered': '✓ Registered',
  'event.officialLink': 'Register on the official site ↗',
  'event.close': 'Close',

  // Calendar page
  'calendar.title': 'Calendar',
  'calendar.subtitle': 'All events of the season.',
  'calendar.count': '{count} event in selection',
  'calendar.countPlural': '{count} events in selection',
  'calendar.empty': 'No event matches your filters.',
  'calendar.filter.type.label': 'Type',
  'calendar.filter.type.all': 'All',
  'calendar.filter.country.label': 'Country',
  'calendar.filter.country.all': 'All countries',
  'calendar.filter.period.label': 'Period',
  'calendar.filter.period.all': 'All',
  'calendar.filter.period.upcoming': 'Upcoming',
  'calendar.filter.period.past': 'Past',
  'calendar.filter.reset': 'Reset',
  'calendar.search.label': 'Search',
  'calendar.search.placeholder': 'Event name or city…',
  'calendar.search.clear': 'Clear',

  // My Season page
  'season.title': 'My season',
  'season.subtitle': 'Events you marked as "I\'m going".',
  'season.upcoming.title': 'Upcoming',
  'season.past.title': 'Past',
  'season.empty.upcoming': 'No upcoming events. Hit "Register" on the map or calendar.',
  'season.empty.past': 'No past events yet.',

  // Event detail page (Phase 3)
  'detail.back': '← Back',
  'detail.detailButton': 'My recap',
  'detail.section.summary': 'Recap',
  'detail.section.result': 'Result',
  'detail.section.matches': 'Matches',
  'detail.section.expenses': 'Expenses',
  'detail.notFound': 'Event not found.',
  'detail.summary.spent': 'Spent',
  'detail.summary.prizes': 'Prizes',
  'detail.summary.net': 'Net balance',
  'detail.summary.noResult': 'No result recorded for this event.',
  'detail.result.placement': 'Placement',
  'detail.result.totalPlayers': 'Total players',
  'detail.result.deckName': 'Deck name',
  'detail.result.leaderPlayed': 'Leader played',
  'leaderPicker.searchPlaceholder': 'Search a leader…',
  'leaderPicker.change': 'Change',
  'leaderPicker.viewNormalFace': 'Normal face',
  'leaderPicker.viewAwakenedFace': 'Awakened face',
  'detail.result.prizes': 'Prizes (€)',
  'detail.result.notes': 'Notes',
  'detail.result.save': 'Save',
  'detail.result.saving': 'Saving…',
  'detail.result.saved': 'Result saved.',
  'detail.result.delete': 'Clear result',
  'detail.result.deleteConfirm': 'Clear the result (and all matches)?',
  'detail.result.errors.required': 'Required fields missing.',
  'detail.result.errors.deckLeaderConflict': 'This deck name is already used with leader {leader} — choose a different name, or select that leader instead.',
  'detail.matches.empty': 'No matches logged yet.',
  'detail.matches.add': 'Add a match',
  'detail.matches.round': 'Round',
  'detail.matches.opponentLeader': 'Opponent Leader',
  'detail.matches.opponentName': 'Opponent name',
  'detail.matches.result': 'Result',
  'detail.matches.result.win': 'Win',
  'detail.matches.result.loss': 'Loss',
  'detail.matches.result.draw': 'Draw',
  'detail.matches.result.bye': 'Bye',
  'detail.matches.record': '{w}W – {l}L – {d}D – {b}B',
  'detail.matches.delete': 'Delete',
  'detail.matches.errors.required': 'Leader and result required.',
  'detail.matches.cta': 'Add',
  'detail.matches.update': 'Save',
  'detail.matches.cancelEdit': 'Cancel',
  'detail.expenses.empty': 'No expenses.',
  'detail.expenses.add': 'Add an expense',
  'detail.expenses.category': 'Category',
  'detail.expenses.amount': 'Amount (€)',
  'detail.expenses.notes': 'Note',
  'detail.expenses.total': 'Total spent',
  'detail.expenses.delete': 'Delete',
  'detail.expenses.errors.required': 'Category and amount required.',
  'detail.expenses.cta': 'Add',
  'detail.expenses.cat.Transport': 'Transport',
  'detail.expenses.cat.Hotel': 'Hotel',
  'detail.expenses.cat.Entry': 'Registration',
  'detail.expenses.cat.Food': 'Food',
  'detail.expenses.cat.Other': 'Other',

  // Collections
  'collection.list.title': 'My collections',
  'collection.list.add': 'New collection',
  'collection.list.empty': 'No collections yet.',
  'collection.list.cards': '{count} cards',
  'collection.editor.back': '← Back',
  'collection.editor.namePlaceholder': 'Collection name',
  'collection.editor.myCollection': 'My collection',
  'collection.editor.loading': 'Loading…',
  'collection.editor.summary': '{count} cards · €{total}',
  'collection.editor.empty': 'No cards added yet.',
  'collection.editor.save': 'Save collection',
  'collection.detail.loading': 'Loading…',
  'collection.detail.notFound': 'Collection not found.',
  'collection.detail.edit': 'Edit',
  'collection.detail.delete': 'Delete',
  'collection.detail.deleteConfirm': 'Delete this collection? This action cannot be undone.',
  'collection.detail.summary': '{count} cards · €{total}',
  'collection.detail.empty': 'This collection has no cards yet.',
  'collection.errors.nameRequired': 'Collection name is required.',
  'collection.errors.saveFailed': 'Something went wrong, try again.',
  'cardGrid.searchPlaceholder': 'Search a card…',
  'cardGrid.allTypes': 'All types',
  'cardGrid.allColors': 'All colors',
  'cardGrid.allSeries': 'All series',
  'cardGrid.allRarities': 'All rarities',
  'cardGrid.empty': 'No cards found.',
  'cardGrid.prev': 'Previous',
  'cardGrid.next': 'Next',
  'cardDetail.quantity': 'Quantity',
  'cardDetail.price': 'Unit price (€)',
  'cardDetail.add': 'Add',
  'cardDetail.remove': 'Remove',
  'cardDetail.language': 'Language',
  'cardDetail.languageUnset': 'Unspecified',
  'cardDetail.languageFr': 'French',
  'cardDetail.languageEn': 'English',
  'cardDetail.viewNormalFace': 'Normal face',
  'cardDetail.viewAwakenedFace': 'Awakened face',

  // Budget page (Phase 4)
  'budget.title': 'Budget',
  'budget.subtitle': 'Aggregated recap of your season spending and prizes.',
  'budget.year': 'Year',
  'budget.totals.spent': 'Spent',
  'budget.totals.prizes': 'Prizes',
  'budget.totals.net': 'Net',
  'budget.byCategory.title': 'By category',
  'budget.byCategory.empty': 'No expense for this year.',
  'budget.byEvent.title': 'By event',
  'budget.byEvent.empty': 'No event budgeted for this year.',
  'budget.byEvent.col.event': 'Event',
  'budget.byEvent.col.date': 'Date',
  'budget.byEvent.col.spent': 'Spent',
  'budget.byEvent.col.prizes': 'Prizes',
  'budget.byEvent.col.net': 'Net',

  // Dashboard page (Phase 4)
  'dashboard.title': 'Dashboard',
  'dashboard.subtitle': 'Your season at a glance.',
  'dashboard.year': 'Year',
  'dashboard.empty': 'No data for this year. Add a result on an event detail page.',
  'dashboard.wr.title': 'Global win rate',
  'dashboard.wr.played': '{count} matches played',
  'dashboard.wr.record': '{w}W – {l}L – {d}D – {b}B',
  'dashboard.best.title': 'Best placement',
  'dashboard.best.empty': 'No result recorded yet.',
  'dashboard.best.outOf': 'of {total}',
  'dashboard.cities.title': 'Cities visited',
  'dashboard.cities.count': '{cities} cities · {countries} countries',
  'dashboard.cities.empty': 'No event played this year.',
  'dashboard.matchup.title': 'Matchup breakdown',
  'dashboard.matchup.deckFilter': 'My leader',
  'dashboard.matchup.all': 'All',
  'dashboard.matchup.empty': 'No match for this leader.',
  'dashboard.matchup.col.opp': 'Opponent',
  'dashboard.matchup.col.n': 'N',
  'dashboard.matchup.col.wr': 'WR',
  'dashboard.deck.title': 'Win rate by deck',
  'dashboard.deck.empty': 'No deck played this year.',
  'dashboard.deck.col.deck': 'Deck',
  'dashboard.deck.col.leader': 'Leader',
  'dashboard.deck.col.n': 'N',
  'dashboard.deck.col.wr': 'WR',
  'dashboard.money.title': 'Financial recap',
  'dashboard.money.seeDetail': 'See detail →',
  'dashboard.next.title': 'Upcoming events',
  'dashboard.next.empty': 'No upcoming events.',
};

const DICTS: Record<Lang, Record<string, string>> = { fr: FR, en: EN };

@Injectable({ providedIn: 'root' })
export class I18nService {
  private readonly _lang = signal<Lang>(this.readLang());
  readonly lang: Signal<Lang> = this._lang.asReadonly();

  setLang(lang: Lang): void {
    this._lang.set(lang);
    try {
      localStorage.setItem(LANG_KEY, lang);
    } catch {
      /* ignore */
    }
  }

  t(key: string, params?: Record<string, string | number>): string {
    const dict = DICTS[this._lang()];
    let value = dict[key] ?? key;
    if (params) {
      for (const [k, v] of Object.entries(params)) {
        value = value.replace(`{${k}}`, String(v));
      }
    }
    return value;
  }

  private readLang(): Lang {
    try {
      const raw = localStorage.getItem(LANG_KEY);
      if (raw === 'en' || raw === 'fr') return raw;
    } catch {
      /* ignore */
    }
    const navLang = typeof navigator !== 'undefined' ? navigator.language : '';
    return navLang.toLowerCase().startsWith('en') ? 'en' : 'fr';
  }
}
